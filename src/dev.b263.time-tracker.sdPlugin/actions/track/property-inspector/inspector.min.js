...
$PI.onConnected(
  ({ actionInfo, appInfo, connection, messageType, port, uuid }) => {
    ...
    const onFormChangeDebounced = Utils.debounce(200, () => {
      const settings = Utils.getFormValue(
        document.getElementById("property-inspector")
      );
      if (
        settings["project_id"] !== State.get(StateKey.settings)["project_id"]
      ) {
        loadTasks(settings["project_id"]);
      }
...
function onSettingsReceived(settings, updateForm = false) {
  console.log("onSettingsReceived", { settings, updateForm });
  State.set(StateKey.settings, settings);
  if (updateForm) {
    Utils.setFormValue(settings, document.getElementById("property-inspector"));
  }
}
...
async function loadProjects() {
  ...
  options += result.data
    .map((project) => {
      const selected =
        +State.get("settings")["project_id"] === +project.id ? "selected" : "";
      return `<option ${selected} value="${project.id}">${project.name}</option>`;
    })
    .join("");
  document.querySelector("[name='project_id']").innerHTML = options;
}
...
async function loadTasks(projectId) {
  ...
  console.log("loadTasks", JSON.stringify(tasks));
  let options = '<option value="">Select task (optional)</option>';
  options += tasks
    .map((task) => {
      const selected =
        +State.get("settings")["task_id"] === +task.id ? "selected" : "";
      return `<option ${selected} value="${task.id}">${task.name}</option>`;
    })
    .join("");
  document.querySelector("[name='task_id']").innerHTML = options;
}
